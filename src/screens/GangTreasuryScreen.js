...
window.GangTreasuryScreen = function GangTreasuryScreen({ cu, gangs, allUsers, setCurrentPage }) {
  const S = {
    load: (k, def) => { try { const v = localStorage.getItem("us_gangt_"+k); return v ? JSON.parse(v) : def; } catch { return def; } },
    save: (k, v)   => { try { localStorage.setItem("us_gangt_"+k, JSON.stringify(v)); } catch {} },
  };

  const [tab, setTab]     = React.useState("treasury");
  const [weapons, setWeapons] = React.useState(()=>S.load("weapons",{}));
  const [txLog, setTxLog] = React.useState(()=>S.load("txlog",{}));
  const [contracts, setContracts] = React.useState(()=>S.load("contracts",{}));
  ...
  const isLeader = myGang?.leader===cu?.username;
  ...
  const myWeapons = weapons[myGang?.id] || {};
  const totalPower = WEAPONS_CATALOG.reduce((a,w) => a + (myWeapons[w.id]||0)*w.power, 0);
  ...
  const buyWeapon = (weapon) => {
    ...
    const gangBank = myGang.bank||0;
    if(gangBank < weapon.price) return showMsg(`Yetersiz bakiye. Mevcut: ${fmtMoney(gangBank)}`, "error");
    const updW = {...weapons,[myGang.id]:{...myWeapons,[weapon.id]:(myWeapons[weapon.id]||0)+1}};
    setWeapons(updW); S.save("weapons",updW);
    const tx = {type:"buy",item:weapon.name,amount:-weapon.price,timestamp:Date.now()};
    const updTx = {...txLog,[myGang.id]:[(tx),...(txLog[myGang.id]||[])].slice(0,30)};
    setTxLog(updTx); S.save("txlog",updTx);
    showMsg(`${weapon.name} satın alındı! +${weapon.power} güç ✓`,"success");
  };
  ...
  const addContract = () => {
    ...
    setContracts(updC); S.save("contracts",updC);
        ...
        <div>
          <div style={{...card,background:"linear-gradient(135deg,rgba(239,68,68,0.08),rgba(0,0,0,0))"}}>
            <div style={{textAlign:"center",padding:"0.75rem"}}>
              <div style={{fontSize:"0.72rem",color:"#5E7390",marginBottom:"0.3rem",textTransform:"uppercase",letterSpacing:"0.07em"}}>Mevcut Kasa</div>
              <div style={{fontFamily:"JetBrains Mono,monospace",fontWeight:900,fontSize:"1.8rem",color:"#EF4444"}}>{fmtMoney(myGang.bank||0)}</div>
      ...
      {tab==="weapons"&&(
        <div>
          <div style={card}>
            ...
            {WEAPONS_CATALOG.filter(w=>(myWeapons[w.id]||0)>0).length===0&&(
              <div style={{textAlign:"center",color:"#5E7390",padding:"0.75rem",fontSize:"0.82rem"}}>Henüz silah yok.</div>
            )}
            {WEAPONS_CATALOG.filter(w=>(myWeapons[w.id]||0)>0).map(w=>(
              <div key={w.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"0.45rem 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
                ...
                <div style={{textAlign:"right"}}>
                  <span style={{fontWeight:700,color:"#EF4444"}}>x{myWeapons[w.id]}</span>
                  <span style={{fontSize:"0.68rem",color:"#5E7390",marginLeft:"0.4rem"}}>(+{(myWeapons[w.id]||0)*w.power} güç)</span>
                </div>
              </div>
            ))}
          ...
          {isLeader&&(
            <div style={card}>
              ...
              <div style={{fontSize:"0.75rem",color:"#5E7390",marginBottom:"0.75rem"}}>Kasa: {fmtMoney(myGang.bank||0)}</div>
              {WEAPONS_CATALOG.map(w=>(
                <div key={w.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"0.55rem 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
                  ...
                  <button
                    ...
                    style={{padding:"0.35rem 0.75rem",fontSize:"0.75rem",minHeight:32,opacity:(myGang.bank||0)>=w.price?1:0.4}}
                    onClick={()=>buyWeapon(w)}
                    disabled={(myGang.bank||0)<w.price}
                  >
                    Satın Al
                  </button>
                </div>
          ...
          <div style={card}>
            ...
            {(txLog[myGang?.id]||[]).map((tx,i)=>(
              <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"0.45rem 0",borderBottom:"1px solid rgba(255,255,255,0.04)",fontSize:"0.8rem"}}>
                <div>
                  <div style={{color:"#ddd"}}>{tx.item}</div>
                  <div style={{fontSize:"0.65rem",color:"#5E7390"}}>{new Date(tx.timestamp).toLocaleDateString("tr-TR")}</div>
                </div>
                <span style={{fontWeight:700,fontFamily:"JetBrains Mono,monospace",color:tx.amount>=0?"#10B981":"#EF4444"}}>{tx.amount>=0?"+":""}{fmtMoney(tx.amount)}</span>
              </div>
            ))}
          </div>
        </div>
